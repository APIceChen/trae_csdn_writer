...
- **来源**: https://blog.csdn.net/Nova_511/article/details/161144162
- **作者**: Nova_511
- **抓取时间**: 2026-05-28
- **分类**: 技术教程 / Python 基础知识
...
当 `count` 超过 `5` 时，就说明数据已经取完，于是抛出 `StopIteration`。
...
既然 `MyIter` 已经实现了 `__iter__()` 和 `__next__()`，那么它也可以直接用于 `for` 循环：
...
核心结论如下：
- `for` 循环底层依赖 `iter()` 和 `next()`；
- 可迭代对象是可以被 `iter()` 处理的对象；
- 迭代器是可以被 `next()` 不断取值的对象；
- 迭代器通过 `__iter__()` 和 `__next__()` 实现；
